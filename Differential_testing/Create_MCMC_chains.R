#####################################################################
#### MCMC simulations for selected cells ############################
#### These Chains will be used for DE and DV testing ################
#####################################################################
...
# For DE and DV testing, the MCMC should be run on specific conditions inpedendently
# Here we chose the B6 young naive cells for demonstration
cells <- colnames(input)[grepl("SS51_naive", colnames(input)) | grepl("SS52_naive", colnames(input))]
bio.g.1 <- bio.g.1[,cells]
ERCC.1 <- ERCC[,cells]
...
### Save the MCMC_output and Data for DE and DV testing
